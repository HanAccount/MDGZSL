...
class CLASSIFIER:
    ...
    def __init__(self, opt, _train_X, _train_Y, data_loader, test_seen_feature, test_unseen_feature, _nclass, _cuda, _lr=0.001, _beta1=0.5, _nepoch=20,
                 _batch_size=100,  generalized=True, netFR=None, dec_hidden_size=4096, dec_size=4096, MCA=True):
        self.opt = opt
        ...
        # self.train_X = torch.cat((self.train_X, self.train_att), 0)
        # self.test_seen_feature = torch.cat((self.test_seen_feature, self.test_seen_att), 0)
        # self.test_unseen_feature = torch.cat((self.test_unseen_feature, self.test_unseen_att), 0)
        # if self.netFR:
        #     self.netFR.eval()
        #     # input_dim:2984
    ...
    def compute_fear_out(self, test_X, new_size):
        ...
        for i in range(0, ntest, self.opt.batchsize):
            end = min(ntest, start + self.opt.batchsize)
            if self.cuda:
                with torch.no_grad():
            ...
            new_test_X[start:end] = torch.cat([inputX, self.train_att]).data.cpu()
            start = end
        return new_test_X
